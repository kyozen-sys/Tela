...
tela::Window::create(int width, int height, std::string_view title, bool resizable) {
    return std::make_unique<tela::backend::wayland::WaylandWindow>(width, height, title, resizable);
}
...
void on_wl_global(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t) {
    auto* impl = static_cast<tela::backend::wayland::WaylandWindowImpl*>(data);
    ...
    if (strcmp(interface, wl_seat_interface.name) == 0) {
        ...
        wl_seat_add_listener(impl->wwl_seat, &tela::backend::wayland::wwl_seat_listener, impl);
    }
...
namespace tela::backend::wayland {
